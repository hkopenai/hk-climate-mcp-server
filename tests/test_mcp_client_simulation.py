...
class TestMCPClientSimulation(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        print("[DEBUG] Starting MCP server subprocess for HTTP communication...")
        ...
        print("[DEBUG] MCP server subprocess started. Giving it a moment to start up and listen on HTTP...")
        ...
        for _ in range(10):
            try:
                with socket.create_connection(("127.0.0.1", 8000), timeout=1):
                    print("[DEBUG] Server is listening on port 8000.")
                    break
            except OSError as e:
                print(f"[DEBUG] Waiting for server to start: {e}")
                time.sleep(1)
        else:
            self.fail("Server did not start listening on port 8000 in time.")

        print(f"[DEBUG] Server setup complete.")

    @classmethod
    def tearDownClass(cls):
        # Terminate the server process
        if cls.server_process:
            print("[DEBUG] Tearing down: Terminating server process...")
            cls.server_process.terminate()
            cls.server_process.wait(timeout=5)
            if cls.server_process.poll() is None:
                print("[DEBUG] Server process did not terminate, killing...")
            ...
            if cls.server_process.stderr:
                ...
                if stderr_output:
                    print(f"Server stderr (remaining):\n{stderr_output}")
                else:
                    print("Server stderr (remaining): (empty)")
            print("[DEBUG] Tear down complete.")

    def test_get_current_weather_tool(self):
        print("[DEBUG] Testing 'get_current_weather' tool...")
        try:
            async def _run_test():
                async with streamablehttp_client(self.SERVER_URL) as (read_stream, write_stream, _):
                    async with ClientSession(read_stream, write_stream) as session:
                        ...
                        print(f"[DEBUG] 'get_current_weather' tool response: {response}")
                        ...
                        print("[DEBUG] 'get_current_weather' tool test passed.")
